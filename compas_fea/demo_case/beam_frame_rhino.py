...
mdl = Structure(name='beam_frame', path=TEMP_PATH)

# Elements
rhino.add_nodes_elements_from_layers(
    mdl, line_type='BeamElement', layers='elset_beams')

# Sets
rhino.add_sets_from_layers(
    mdl, layers=['nset_pins', 'nset_load_v', 'nset_load_h', 'nset_rollers'])
...
# Properties
mdl.add(Properties(name='ep_beam', material='mat_steel',
        section='sec_pipe', elset='elset_beams'))
...
# Run
mdl.analyse_and_extract(software='opensees', fields=['u', 'rf', 'sf', 'sm'])
